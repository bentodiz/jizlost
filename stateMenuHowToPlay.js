...
StateMenuHowToPlay.prototype.Draw = function()
{
	...
	/*
	var descr = ["Clear the number of lines indicated on each enemy!", "You get penalized for each remaining piece after its destruction!"];
	for (var i = 0; i < descr.length; i++)
	{
		ctx.save();
		ctx.translate(WIDTH / 2, HEIGHT / 2 + 120 + 30 * i);
		ctx.scale(Interpolate_Decelerate((this.animation - 5) / 24.0), Interpolate_Decelerate((this.animation - 5) / 24.0));
		ctx.fillStyle = "white";
		ctx.font = "20px Impact";
		ctx.textAlign = "center";
		ctx.textBaseline = "middle";
		ctx.fillText(descr[i], 0, 0);
		ctx.restore();
	}*/
